			...
			<view class="slide-box" v-for="(item, index) in listData" :key="index">
				<view class="slide-list" @touchstart="touchStart($event, index)" @touchend="touchEnd($event, index)" @touchmove="touchMove($event, index)"
				 @tap="recover(index)" :style="{ transform: 'translate3d(' + item.slide_x + 'px, 0, 0)' }">
					<view class="now-message-info" hover-class="uni-item--hover" @click="clickItemMethod(item)">
						...
						<view class="list-right">
							...
							<view class="list-label" v-if="item.label">{{ item.label }}</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			TherGropTypeList(data,type) {
				console.log('this.listData',this.listData)
				if(type==='del'){
				const index = this.listData.findIndex(i => i.id === data)
